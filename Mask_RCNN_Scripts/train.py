...
#     IMAGE_MIN_DIM = 256
#     STEPS_PER_EPOCH = 3341 if sys.argv[1] == 'svhn' else (192/2)
#     VALIDATION_STEPS = 836 if sys.argv[1] == 'svhn' else 24
...
#model.train(train_set, validation_set, learning_rate=config.LEARNING_RATE, epochs=15, layers='heads')


# train weights (output layers or 'heads')
aug = imgaug.augmenters.Sometimes(0.5, [
    imgaug.augmenters.Affine(scale = (0.8,1.2)),
    imgaug.augmenters.MotionBlur(k=(3,5))
])
model.train(train_set, validation_set, learning_rate=config.LEARNING_RATE, epochs=15, layers='heads',augmentation = aug)
